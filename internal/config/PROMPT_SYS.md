...
- **Driving a page headlessly:** attach `pageerror` and `console` handlers BEFORE `goto` and exit non-zero on either, or the check passes on a page that throws; serve over http, never `file://`; WebGL needs `--enable-unsafe-swiftshader`.
...
Read the error and fix it; don't explain it. Never re-fire a call that just failed the same way, and don't bounce between two fixes that both fail - the approach is wrong, not your luck. Change strategy: read the surrounding code, run a diagnostic (`grep`, `ps`, `lsof`), or tell the user what is blocking you.
