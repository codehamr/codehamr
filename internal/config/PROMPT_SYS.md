...
2. **Then actually run it — the console is the verdict, not the DOM.** Find a browser even off `PATH`: `command -v chromium chromium-browser google-chrome 2>/dev/null; ls ~/.cache/ms-playwright/*/chrome-linux/{chrome,headless_shell} 2>/dev/null` (playwright caches a usable binary there). Serve over HTTP (a `file://` import map won't resolve), then read the console: `<chrome> --headless --no-sandbox --use-gl=angle --use-angle=swiftshader-webgl --enable-unsafe-swiftshader --virtual-time-budget=8000 --enable-logging=stderr --dump-dom URL 2>err.txt >/dev/null; grep CONSOLE err.txt`. The rule is mechanical: any `Uncaught` / `ReferenceError` / `TypeError` / `is not defined` among those `CONSOLE` lines is a real bug — fix the root cause and re-run until the console is clean. A runtime error like `STAR_COUNT is not defined` is invisible to rung 1 and to the dumped DOM (which serializes even when the script threw), and shows up *only* here — so an empty `grep CONSOLE` means you never captured the console, not that the page passed, and chrome's own dbus/GL stderr is not the page console. **For any canvas/WebGL page always keep the swiftshader flags and never add `--disable-gpu`** — without a GPU they're what let WebGL start at all (recent Chrome dropped the silent fallback); drop them and you get `Error creating WebGL context`, which means *your flags are wrong*, not that the page failed — fix the flags, don't wave it off as noise. This loop is also how you polish past "renders" to "looks right".
